...
def run_ga(csv_data, co_r, mut_r):
    ...
    st.warning('GA LOGIC IS A PLACEHOLDER. Replace this demo data with your real algorithm.')
    
    # Check if 'rating' column exists before trying to access it
    rating_mean = csv_data['rating'].mean() if 'rating' in csv_data.columns else 4.0
    
    demo_data = {
        'Time Slot': ['09:00', '10:00', '11:00'],
        'Scheduled Program': ['(DEMO) Program A', '(DEMO) Program B', '(DEMO) Program C'],
        'Rating': [rating_mean, co_r, mut_r] # Just to show it's using the inputs
    }
...
if st.sidebar.button('Run Genetic Algorithm'):
    ...
    try:
        # 
        # <<< THIS IS THE FIX >>>
        # Changed 'program_ratings.csv.csv' to 'program_ratings.csv'
        #
        ratings_data = pd.read_csv('program_ratings.csv')
        ...
        st.error("Error: The CSV file ('program_ratings.csv') was not found. Please make sure it's in the repository and the filename is correct.")
